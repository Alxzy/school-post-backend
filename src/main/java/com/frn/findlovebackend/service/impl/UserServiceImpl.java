...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long register(String userAccount, String userPassword, String checkPassword) {
       // 1.校验数据
        ...
        if(StringUtils.isAnyBlank(userAccount,userPassword,checkPassword)){
            throw new BussinessException(ErrorCode.PARAM_ERROR,"参数为空");
        }
        ...
        synchronized (userAccount.intern()){
            ...
            if(userMapper.selectCount(queryWrapper) > 0){
                throw new BussinessException(ErrorCode.PARAM_ERROR,"账号不能重复");
            }
            // 2.数据加密
            ...
            if(!isSuccess) {
                throw new BussinessException(ErrorCode.SYSTEM_ERROR, "注册失败,数据库发送错误");
            }
            return user.getId();
        }
    }
    ...
    public User login(String userAccount, String userPassword, HttpServletRequest httpServletRequest) {
        ...
        if(user == null){
            ...
            throw new BussinessException(ErrorCode.PARAM_ERROR, "用户不存在或密码错误");
        }
        // 3.用户信息脱敏
    ...
    public void checkAccountAndPassword(String userAccount,String userPassword){
        ...
        if(userAccount.length() < 4){
            throw new BussinessException(ErrorCode.PARAM_ERROR,"账号长度过短");
        }
        // 密码不小于 8 位
        ...
        if(matcher.find()){
            throw new BussinessException(ErrorCode.PARAM_ERROR,"账号不能包含特殊字符");
        }
    }
    ...
    public boolean userLogout(HttpServletRequest request) {
        if (request.getSession().getAttribute(USER_LOGIN_STATE) == null) {
            throw new BussinessException(OPERATION_ERROR, "未登录");
        }
        // 移除登录态
